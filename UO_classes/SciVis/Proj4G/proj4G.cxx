                    ...
                    float h, int nsteps, float *output_locations, float *speeds)
{
    ...
    for (int i =1; i<=nsteps; i++){
        EvaluateVectorFieldAtLocation(tmp_loc, dims, X, Y, F, rv_1);
        speeds[i-1] = sqrt(rv_1[0]*rv_1[0] + rv_1[1]*rv_1[1]); // set the speed at the first output location
        
        // std::cout << rv_1[0] << "\t" << rv_1[1] << std::endl;

        k2_loc[0] = tmp_loc[0] + h/2 * rv_1[0];
        k2_loc[1] = tmp_loc[1] + h/2 * rv_1[1];
        EvaluateVectorFieldAtLocation(k2_loc, dims, X, Y, F, rv_2);

        // std::cout << rv_2[0] << "\t" << rv_2[1] << std::endl;


        k3_loc[0] = tmp_loc[0] + h/2 * rv_2[0];
        k3_loc[1] = tmp_loc[1] + h/2 * rv_2[1];

        EvaluateVectorFieldAtLocation(k3_loc, dims, X, Y, F, rv_3);

        k4_loc[0] = tmp_loc[0] + h * rv_3[0];
        k4_loc[1] = tmp_loc[1] + h * rv_3[1];

        EvaluateVectorFieldAtLocation(k4_loc, dims, X, Y, F, rv_4);
        // std::cout << rv_4[0] << "\t" << rv_4[1] << std::endl;

        // float k1 = speeds[i-1];
                
        float coefficient = 1.0/6.0;

        output_locations[2*i] = tmp_loc[0] + coefficient * h * (rv_1[0] + 2 * rv_2[0] + 2 * rv_3[0] + rv_4[0]);
        // std::cout << output_locations[2*i] << endl;
        output_locations[2*i+1] = tmp_loc[1] + coefficient * h * (rv_1[1] + 2 * rv_2[1] + 2 * rv_3[1] + rv_4[1]);
...
int main()
{
    ...
    rdr->SetFileName("proj4G_data.vtk");
    rdr->Update();
